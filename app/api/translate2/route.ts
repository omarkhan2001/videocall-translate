...
// (optional) protect names/pet-names so they aren't translated
...
const protect = (t: string) => Object.entries(protectMap).reduce((s,[k,v])=>s.replaceAll(k,v), t);
const unprotect = (t: string) => Object.entries(protectMap).reduce((s,[k,v])=>s.replaceAll(v,k), t);

// send one REST call to DeepL
...
export async function POST(req: NextRequest) {
  try {
    ...
    params.append("target_lang", ["EN","EN-GB","EN-US","IT","RU"].includes(tgt) ? tgt : "EN");

    // Decide source:
    // - explicit myLang (not auto) -> use it
    // - else for short/one-word -> force smart guess (DeepL warns short texts can be misdetected)
    // - else omit source_lang to enable auto-detect
    ...
    // If unchanged, retry with RU -> IT -> EN as fallbacks
    const same = translated.trim().toLowerCase() === text.trim().toLowerCase();
    if (same) {
    ...
    return NextResponse.json(
      { error: "Translation failed", detail: e?.message || String(e), version: "deepl-rest-v2" },
      { status: 500 }
    );
  }
}
